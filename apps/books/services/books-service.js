...
const KEY = 'booksDB'
var books = storageService.loadFromStorage(KEY) || null
...
function query(filterBy) {
  if (books) {
    if (filterBy) {
      const { bookName, maxPriceRange } = filterBy
      const filteredBooks = books.filter(book => {
        return book.title.includes(bookName) && book.listPrice.amount <= maxPriceRange
      })
...
function addReview(bookId, review) {
  review = { id: utilsService.makeId(), ...review }
    ...
    .then(res => {
      ...
      return items.map(item => {
        ...
        return {
          ...
          listPrice: {
            currencyCode: 'ILS',
            amount: utilsService.getRandomIntInclusive(10, 200),
          },
...
function addGoogleBook(googleBook) {
  if (books) books.push(googleBook)
  storageService.saveToStorage(KEY, books)
  return Promise.resolve()
}
...
function getNextBookId(bookId) {
  ...
  nextBookIdx = nextBookIdx === books.length ? 0 : nextBookIdx
